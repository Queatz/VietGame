...
export class ItemsController {
  ...
  constructor(private overlay: OverlayController, private map: MapController, private level: LevelController, private scene: Scene, private world: WorldController) {
  ...
  restart() {
    ...
    const listMesh = this.base.clone()
    listMesh.isVisible = true
    const material = new StandardMaterial('list', this.scene)
    material.transparencyMode = Material.MATERIAL_ALPHATEST
    material.emissiveTexture = material.diffuseTexture = new Texture('/assets/list.png', this.scene, false, true, Texture.NEAREST_SAMPLINGMODE)
    material.diffuseTexture.hasAlpha = true
    material.useAlphaFromDiffuseTexture = true
    material.backFaceCulling = false
    material.specularColor = Color3.Black()
    material.linkEmissiveWithDiffuse = true
    listMesh.material = material
    ...
    listMesh.billboardMode = Mesh.BILLBOARDMODE_Y
